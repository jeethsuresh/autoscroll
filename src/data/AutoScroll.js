...
function shouldSticky(x, y) {
  return defaults["stickyScroll"] && /*state.stickyScroll && */math.hypot(x, y) < options["dragThreshold"]
}

function scale(value) {
  return value / defaults["moveSpeed"]
}
...
function mousemove(event) {
  ...
  if (math.hypot(x, y) > defaults["moveThreshold"]) {
    ...
    if (defaults["shouldCap"]) {
      x = math.max(x, defaults["capSpeed"])
      y = math.max(y, defaults["capSpeed"])
    }
...
addEventListener("mousedown", function (e) {
  if (state.scrolling) {
    ...
    if (target != null &&
      ((e.button === 1 && defaults["middleClick"]) ||
        (e.button === 0 && (e.ctrlKey || e.metaKey) && defaults["ctrlClick"])) &&
